// g++ -ansi -pedantic evaluator/common.cpp submission/Particle.cpp submission/Wall.cpp submission/CurvyWall.cpp submission/Cell.cpp submission/MicroOrganism.cpp submission/Circular.cpp submission/Tissue.cpp tests/test4/part9.cpp -o tests/test4/part9
...
int tester() {
	...
	p.push_back(new Particle(-7, 8.59));
	p.push_back(new Particle(-9, 7));
	p.push_back(new Particle(-5, -11.58));
	p.push_back(new Particle(1, -12.135));

	// define cell walls
	vector<Wall*> walls;
	for (int i=0; i < p.size()-1; i++)
		walls.push_back(new CurvyWall(*p[i], *p[i+1], Particle(-1,-1)));
	walls.push_back(new Wall(*p[4], *p[0]));
	...
	MicroOrganism* mo = new Circular(10, 11.31);
