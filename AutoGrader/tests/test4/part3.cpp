// g++ -ansi -pedantic evaluator/common.cpp submission/Particle.cpp submission/Wall.cpp submission/CurvyWall.cpp submission/Cell.cpp submission/MicroOrganism.cpp submission/NucleoBase.cpp submission/Triangular.cpp submission/Tissue.cpp tests/test4/part3.cpp -o tests/test4/part3
...
int tester() {
	...
	vector<Particle*> c;
	c.push_back(new Particle(1.5, 0.5));	// center-1
	c.push_back(new Particle(-1.5, 2));		// center-2
	c.push_back(new Particle(-3, 0.5));		// center-3

	// define cell walls
	vector<Wall*> walls;
	for (int i=0; i < p.size(); i++)
		walls.push_back(new CurvyWall(*p[i], *p[(i+1)%p.size()], *c[i]));

	// define triangular microorganism walls
	vector<Wall*> walls2;
	walls2.push_back(new Wall(4.24));
	walls2.push_back(new Wall(9.0));
	walls2.push_back(new Wall(6.708));
