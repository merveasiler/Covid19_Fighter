// g++ -ansi -pedantic evaluator/common.cpp submission/Particle.cpp submission/Wall.cpp submission/CurvyWall.cpp submission/Cell.cpp submission/Tissue.cpp tests/test3/part4.cpp -o tests/test3/part4
...
int tester() {
	...
	vector<Particle*> p;
	p.push_back(new Particle(-1, 5));	// vertex-1
	p.push_back(new Particle(2, 1));	// vertex-2
	p.push_back(new Particle(6, 9));	// vertex-3
	...
	if (newCellWall.size() != 3)
		...
		if (newCellWall[0]->GetInitialParticle() == *p[0] && newCellWall[1]->GetInitialParticle() == *p[1] && newCellWall[2]->GetInitialParticle() == *p[2])
			result = 1;
		else if (newCellWall[1]->GetInitialParticle() == *p[0] && newCellWall[2]->GetInitialParticle() == *p[1] && newCellWall[0]->GetInitialParticle() == *p[2])
			result = 1;
		else if (newCellWall[2]->GetInitialParticle() == *p[0] && newCellWall[0]->GetInitialParticle() == *p[1] && newCellWall[1]->GetInitialParticle() == *p[2])
			result = 1;
		else
			result = 0;
	}
