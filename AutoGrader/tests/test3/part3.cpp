// g++ -ansi -pedantic evaluator/common.cpp submission/Particle.cpp submission/Wall.cpp submission/CurvyWall.cpp submission/Cell.cpp submission/Tissue.cpp tests/test3/part3.cpp -o tests/test3/part3
...
int tester() {
	...
	vector<Particle*> p;
	p.push_back(new Particle(1, 7));
	p.push_back(new Particle(-3, 5.47));
	p.push_back(new Particle(-4.91, 0));
	p.push_back(new Particle(1, -5));
	p.push_back(new Particle(4.316, -4));
	p.push_back(new Particle(6.196, -2));
	p.push_back(new Particle(5, 5.47));
	Particle* c = new Particle(1, 1);	// center
	...
	if (newCellWall.size() != 1)
		...
		if (abs (q.FindDistance(*c) - 6) < 0.01 && newCellWall[0]->GetFinalParticle() == q && ((CurvyWall*)newCellWall[0])->GetCenter() == *c)
			result = 2;
		else
			result = 0;
	}
