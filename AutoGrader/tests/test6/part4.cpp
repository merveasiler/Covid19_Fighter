// g++ -ansi -pedantic evaluator/common.cpp submission/Particle.cpp submission/Wall.cpp submission/CurvyWall.cpp submission/Cell.cpp submission/MicroOrganism.cpp submission/Circular.cpp submission/Tissue.cpp tests/test6/part4.cpp -o tests/test6/part4
...
int tester() {
	...
	// define circle cell walls
	vector<Particle*> p;
	p.push_back(new Particle(7, 7));
	p.push_back(new Particle(-9, 7));
	p.push_back(new Particle(0, 10.269));
	...
	if ( ((Circular*)mo)->DoesContain(Particle(-3, -3)) )
		result += 2;
	if ( ((Circular*)mo)->DoesContain(Particle(12, 12)) == false )
		result += 2;
